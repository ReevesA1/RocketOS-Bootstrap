...
user.services.protonvpn-cli = {
  ...
  serviceConfig = {
    ...
    ExecStart = "${pkgs.bash}/bin/bash -c 'while true; do if ${pkgs.protonvpn-cli}/bin/protonvpn-cli status | ${pkgs.gnugrep}/bin/grep -q \"No active Proton VPN connection.\"; then ${pkgs.protonvpn-cli}/bin/protonvpn-cli killswitch --off && ${pkgs.protonvpn-cli}/bin/protonvpn-cli killswitch --on && ${pkgs.protonvpn-cli}/bin/protonvpn-cli connect --cc CA && ${pkgs.killall}/bin/killall conky && ${pkgs.conky}/bin/conky; fi; sleep 30; done'";
    Restart = "always";
  };
};
