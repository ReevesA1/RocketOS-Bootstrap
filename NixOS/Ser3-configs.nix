...
#shutdown -c WORKS - it helps with pc rebooting after resuming from suspend
...
#!SystemD Services
  ...
  timers."conky-stop-multiple" = {
    ...
    timerConfig = {
      OnCalendar = "*-*-* *:*:00"; #every minute might be to much ?
      Unit = "conky-stop-multiple.service";
    };
  };
  services."conky-stop-multiple" = {
    script = ''
    ${pkgs.powershell}/bin/pwsh $HOME/Timers/SER3/conky-stop-multiple.ps1
    '';
    serviceConfig = {
      User = "rocket";
